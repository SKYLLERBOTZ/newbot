...
async function LoadDataBase(Ditss, m) {
	try {
...
async function MessagesUpsert(Ditss, message, store) {
	try {
		...
		Ditss.readMessages([msg.key])
		} else return
		}		
		if (!msg.message) return
		if (!Ditss.public && !msg.key.fromMe && message.type === 'notify') return
		if (global.db.settings.autoread && global.db.settings.autoread == true) Ditss.readMessages([msg.key])
		if (global.db.settings.autotyping && global.db.settings.autotyping == true && !msg.key.fromMe) Ditss.sendPresenceUpdate('composing', msg.key.remoteJid)
		const m = await Serialize(Ditss, msg, store)
		if (m.isBaileys) return
		require('../case.js')(Ditss, m, message, store);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			...
			apb.key = msg.key
			apb.key.fromMe = areJidsSameUser(m.sender, Ditss.user.id);
			...
			Ditss.ev.emit('messages.upsert', pbr);
		}
	} catch (e) {
		throw e;
	}
...
async function Solving(Ditss, store) {
	Ditss.public = true
	
	Ditss.serializeM = (m) => MessagesUpsert(Ditss, m, store)
	...
	Ditss.getName = (jid, withoutContact  = false) => {
		const id = Ditss.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || Ditss.groupMetadata(id) || {};
	...
	Ditss.sendContactV2 = async (jid, kon, desk = "Developer Bot", quoted = '', opts = {}) => {
let list = []
for (let i of kon) {
list.push({
...
Ditss.sendButtons = async (jid, buttons, quoted, opts = {}) => {
	...
	Ditss.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await Ditss.query({
			tag: 'iq',
			attrs: {
	...
	Ditss.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return Ditss.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime && mime === 'application/pdf') {
				return Ditss.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted });
			} else if (mime && mime.includes('image')) {
				return Ditss.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted });
			} else if (mime && mime.includes('video')) {
				return Ditss.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted });
			} else if (mime && mime.includes('audio')) {
				return Ditss.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted });
			}
		}
	...
	Ditss.sendTextMentions = async (jid, text, quoted, options = {}) => Ditss.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	Ditss.sendAsSticker = async (jid, path, quoted, options = {}) => {
	...
	Ditss.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await Ditss.getFile(path, true);
		...
		await Ditss.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
...
async function Serialize(Ditss, m, store) {
	const botNumber = await Ditss.decodeJid(Ditss.user.id)
	...
	if (m.key) {
		...
		m.sender = await Ditss.decodeJid(m.fromMe && Ditss.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			m.metadata = m.isGroup ? (await Ditss.groupMetadata(m.chat).catch(_ => [{}]) || [{}]) : [{}]
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.getQuotedObj = async () => {
				...
				return await Serialize(Ditss, q, store)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(Ditss.decodeJid(m.msg?.contextInfo?.participant), Ditss.decodeJid(Ditss?.user?.id)),
			...
			m.quoted.delete = () => {
				Ditss.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
		...
		try {
			if (/^https?:\/\//.test(text)) {
				...
				if (/gif|image|video|audio|pdf/i.test(mime)) {
					return Ditss.sendFileUrl(chatId, text, caption, quoted, options)
				} else {
					return Ditss.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
				}
			} else {
				return Ditss.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
			}
