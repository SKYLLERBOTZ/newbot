...
const Hikaaa = makeWASocket({
logger: pino({ level: "silent" }),
printQRInTerminal: !global.usePairingCode,
auth: state,
browser: ['Ubuntu', 'Edge', '22.04.4'], // for this issues 
});
  if (global.usePairingCode && !Hikaaa.authState.creds.registered) {
const phoneNumber = await question('𝙼𝚊𝚜𝚞𝚔𝚊𝚗 𝙽𝚘𝚖𝚎𝚛 𝚈𝚊𝚗𝚐 𝙰𝚔𝚝𝚒𝚏 𝙰𝚠𝚊𝚕𝚒 𝙳𝚎𝚗𝚐𝚊𝚗 𝟼𝟸 :\n');
let code = await Hikaaa.requestPairingCode(phoneNumber);
code = code?.match(/.{1,4}/g)?.join("-") || code;
console.log(`𝙲𝙾𝙳𝙴 𝙿𝙰𝙸𝚁𝙸𝙽𝙶 :`, code);
}
{
store.bind(Hikaaa.ev)

Hikaaa.ev.on('messages.upsert', async chatUpdate => {
try {
mek = chatUpdate.messages[0]
if (!mek.message) return
mek.message = (Object.keys(mek.message)[0] === 'ephemeralMessage') ? mek.message.ephemeralMessage.message : mek.message
if (mek.key && mek.key.remoteJid === 'status@broadcast') return
if (!Hikaaa.public && !mek.key.fromMe && chatUpdate.type === 'notify') return
if (mek.key.id.startsWith('BAE5') || (mek.key.id.startsWith('3EB0') && mek.key.id.length == 22)) return
m = smsg(Hikaaa, mek, store)
require("./stik/Yuhao")(Hikaaa, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
Hikaaa.getName = (jid, withoutContact= false) => {
id = Hikaaa.decodeJid(jid)
withoutContact = Hikaaa.withoutContact || withoutContact 
let v
if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
v = store.contacts[id] || {}
if (!(v.name || v.subject)) v = Hikaaa.groupMetadata(id) || {}
resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
})
else v = id === '0@s.whatsapp.net' ? {
id,
name: 'WhatsApp'
} : id === Hikaaa.decodeJid(Hikaaa.user.id) ?
Hikaaa.user :
(store.contacts[id] || {})
...
Hikaaa.public = true
global.welcome = true
Hikaaa.serializeM = (m) => smsg(Hikaaa, m, store);
Hikaaa.ev.on('connection.update', (update) => {
...
Hikaaa.end(`Unknown DisconnectReason: ${reason}|${connection}`);
}
} else if (connection === 'open') {
console.log('[Connected] ' + JSON.stringify(Hikaaa.user.id, null, 2));
}
});

Hikaaa.ev.on('creds.update', saveCreds)

Hikaaa.sendText = (jid, text, quoted = '', options) => Hikaaa.sendMessage(jid, { text: text, ...options }, { quoted })

//=========================================\\
Hikaaa.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
...
//=========================================\\
Hikaaa.sendTextWithMentions = async (jid, text, quoted, options = {}) => Hikaaa.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
//=========================================\\
Hikaaa.sendImageAsSticker = async(jid, path, quoted, options = {}) => {
...
await Hikaaa.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
return buffer
}
Hikaaa.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await global.getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
let buffer
if (options && (options.packname || options.author)) {
...
Hikaaa.downloadMediaMessage = async (message) => {
let mime = (message.msg || message).mimetype || ''
let messageType = message.mtype ? message.mtype.replace(/Message/gi, '') : mime.split('/')[0]
...
Hikaaa.ev.on('group-participants.update', async (anu) => {
  if (!global.welcome) return
    console.log(anu);
    try {
        let metadata = await Hikaaa.groupMetadata(anu.id);
        ...
        for (let num of participants) {
            ...
            try {
                ppuser = await Hikaaa.profilePictureUrl(num, 'image');
            } catch (err) {
                ppuser = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60';
            }

            try {
                ppgroup = await Hikaaa.profilePictureUrl(anu.id, 'image');
            } catch (err) {
